...
async function lookupOpenProductData(barcode: string): Promise<ProductLookupResult | null> {
  try {
    ...
    console.error('OpenProductData lookup error:', e);
  }
  return null;
}
...
async function lookupUPCItemDB(barcode: string): Promise<ProductLookupResult | null> {
  try {
    ...
    console.error('UPCItemDB lookup error:', e);
  }
  return null;
}
...
export async function GET(request: NextRequest) {
  ...
  if (searchRetailers) {
    console.log(`Barcode ${barcode} not found in public APIs, trying bike retailers...`);
