...
import { checkBookingAccess, checkEmailLicense, incrementEmailAfterSend } from "@/lib/license-guards";
...
export async function GET(req: NextRequest) {
  ...
  if (!prisma) {
    logger.error("[Bookings GET] Prisma not available");
    return NextResponse.json({ error: "prisma_unavailable", message: "Base de données non disponible" }, { status: 503 });
  }
  ...
  try {
    ...
    logger.error("[API] Error fetching bookings:", error);
...
export async function POST(req: NextRequest) {
  ...
  // ⭐ VÉRIFICATION LICENCE: Feature réservations en ligne (Pro)
  const bookingError = await checkBookingAccess();
  ...
  const _tz = cfg.timezone || "Europe/Paris";
  ...
  try {
    ...
    logger.error("booking_customer_link_error", e);
  }
  ...
  try {
    ...
    if (data.email) {
      // Vérifier limite emails avant envoi
      const emailCheckError = await checkEmailLicense();
      ...
      // Incrémenter compteur après envoi réussi
