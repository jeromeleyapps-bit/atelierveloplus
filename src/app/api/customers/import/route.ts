...
export async function POST(req: Request) {
  ...
  const maybeHeader = parseCSVLine(lines[0], delim);
  ...
  if (looksLikeExpectedHeader) {
    ...
    for (const line of dataLines) {
      ...
      try {
        ...
        logger.error('customer_import_line_error', e);
      }
    }
  } else {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        await upsertCustomer({ firstName: firstName ?? null, lastName: lastName ?? null, email: email ?? null, phone: phone ?? null, address1: address1 ?? null, address2: null, zip: zip ?? null, city: city ?? null, country: country ?? null, shipAddress1: null, shipAddress2: null, shipZip: null, shipCity: null, shipCountry: null, notes: notes ?? null });
      } catch (e) {
        logger.error('customer_import_line_error', e);
      }
    }
