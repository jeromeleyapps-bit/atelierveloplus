...
export default function CustomerDetail() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const [customerRes, bikesRes] = await Promise.all([
          fetch(`/api/customers/${id}`, { cache: 'no-store' }),
          fetch(`/api/customers/${id}/bikes`, { cache: 'no-store' })
        ]);
        
        const customerData = await customerRes.json();
        if (!customerRes.ok) throw new Error(customerData?.error || 'load_failed');
        setRow(customerData as Customer);
        
        if (bikesRes.ok) {
          const bikesData = await bikesRes.json();
          setBikesCount(Array.isArray(bikesData) ? bikesData.length : 0);
        }
      } catch (e) {
              ...
              {edit ? (
                <>
                  ...
                  <Button variant="contained" disabled={saving} onClick={async () => {
                    ...
                    } catch (e) { logger.error(e); setToast({ open: true, message: 'Erreur: mise à jour', severity: 'error' }); }
                ...
                <>
                  <Button 
                    variant="outlined" 
                    component={Link} 
                    href={`/customers/${id}/bikes`}
                    startIcon={<DirectionsBikeIcon />}
                    color={bikesCount > 0 ? "success" : "inherit"}
                    sx={{ fontWeight: bikesCount > 0 ? 600 : 400 }}
                  >
                    Vélos {bikesCount > 0 && `(${bikesCount})`}
                  </Button>
