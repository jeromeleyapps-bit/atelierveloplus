...
export async function requestLocal<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  if (!res.ok) {
    ...
    // 401 = Non autorisé, 404 avec user_not_found = JWT invalide
    const shouldLogout = res.status === 401 || (res.status === 404 && res.headers.get('content-type')?.includes('application/json'));
    
    if (shouldLogout && typeof window !== "undefined" && !isAuthPage) {
      ...
      try {
        ...
        if (json.error === "user_not_found" || json.error === "no_user_found") {
          ...
          // Throw pour arrêter l'exécution et éviter erreurs sur undefined
          throw new Error('User not found - redirecting to login');
        }
      } catch (parseError) {
        // Si c'est notre erreur User not found, on la relance
        if (parseError instanceof Error && parseError.message.includes('User not found')) {
          throw parseError;
        }
        // Sinon c'est une erreur de parsing JSON, on continue
      }
      
      // Erreur 401 classique - MAIS vérifier d'abord si on a un token
      if (res.status === 401) {
        ...
        if (!isOnLoginPage) {
          ...
          // On est déjà sur login, ne pas rediriger
...
export type WorkOrder = {
  ...
  // ✅ FIX: Relations Prisma utilisent PascalCase (Customer, CustomerBike)
  Customer?: {
    id: string;
    email?: string | null;
  ...
  // ⚠️ DEPRECATED: Alias camelCase pour rétrocompatibilité (à supprimer plus tard)
  customer?: {
    id: string;
    email?: string | null;
    firstName?: string | null;
...
export type AccountSettings = {
  ...
  city?: string | null;
  country?: string | null;
  shopLogo?: string | null; // Chemin logo atelier (.ico 32x32px)
  legalFooter?: string | null; // Mentions légales pied de page documents
...
export type CatalogItem = {
  ...
  supplierName?: string | null; // Nom du fournisseur (si import CSV)
...
export type SupplierOffer = {
  ...
  catalogItemId?: string | null;  // Lien vers CatalogItem (MON STOCK)
  // Propriétés étendues (depuis SupplierProduct)
  name?: string;
  reference?: string;
  category?: string;
  stock?: number;
...
// Settings
export async function getSetting<T = unknown>(key: string): Promise<{ key: string; value: T | null }> {
  return request(`/settings/${encodeURIComponent(key)}`);
}

export async function setSetting<T = unknown>(key: string, value: T | null): Promise<{ key: string; value: T | null }> {
  // Prisma GlobalSetting.value est String - convertir automatiquement
  const stringValue = value === null ? null : String(value);
  return request(`/settings/${encodeURIComponent(key)}`, { method: 'PUT', body: JSON.stringify({ value: stringValue }) });
}
...
export type AppSettings = {
  ...
  city?: string | null;
  country?: string | null;
  shopLogo?: string | null; // Logo atelier (.ico 32x32px)
  legalFooter?: string | null; // Mentions légales pied de page documents
  // Informations légales
  siret?: string | null;
  tva?: string | null;
...
export async function adminGetStats(): Promise<{ totalUsers: number; activeTickets: number; pendingInvoices: number; dbSize: string; lastBackup: string }> {
...
export async function adminImportBackup(data: { backup: Record<string, unknown>; wipeFirst: boolean }): Promise<{ success: boolean; message?: string }> {
...
export async function adminGetSystemSettings(): Promise<unknown> {
  return requestLocal(`/admin/system-settings`);
}

export async function adminUpdateSystemSettings(data: { setting: string; value: boolean }): Promise<unknown> {
  return requestLocal(`/admin/system-settings`, {
    method: 'PUT',
    body: JSON.stringify(data)
  });
...
export async function getStatsSummary(params?: { from?: string; to?: string }): Promise<unknown> {
  const query = new URLSearchParams();
  if (params?.from) query.set('from', params.from);
  if (params?.to) query.set('to', params.to);
  const qs = query.toString();
...
export async function setWorkOrderAppointment(workOrderId: string, data: { appointmentDate: string; duration?: number }): Promise<unknown> {
...
export async function updateWorkOrderAppointment(workOrderId: string, data: { appointmentDate: string; duration?: number }): Promise<unknown> {
  return requestLocal(`/workorders/${workOrderId}/appointment`, {
    method: 'PUT',
    body: JSON.stringify(data)
  });
...
export async function listCalendarEvents(params: { start: string; end: string }): Promise<unknown[]> {
  return requestLocal(`/calendar/events?start=${encodeURIComponent(params.start)}&end=${encodeURIComponent(params.end)}`);
}

export async function listCalendarBlocks(params: { start: string; end: string }): Promise<unknown[]> {
  return requestLocal(`/calendar/blocks?start=${encodeURIComponent(params.start)}&end=${encodeURIComponent(params.end)}`);
}

export async function listCalendarBookings(params: { start: string; end: string }): Promise<unknown[]> {
  return requestLocal(`/calendar/bookings?start=${encodeURIComponent(params.start)}&end=${encodeURIComponent(params.end)}`);
}

export async function createCalendarEvent(data: { title: string; start: string; end: string; blocksAvail?: boolean }): Promise<unknown> {
...
export async function createCalendarBlock(data: { reason: string; start: string; end: string }): Promise<unknown> {
...
export async function cancelInvoice(invoiceId: string, reason?: string): Promise<unknown> {
...
export async function listCashRegisterEntries(): Promise<unknown[]> {
  return requestLocal(`/cash-register`);
}

export async function createCashRegisterEntry(data: { type: string; amount: number; note?: string; reference?: string }): Promise<unknown> {
  return requestLocal(`/cash-register`, {
    method: 'POST',
    body: JSON.stringify(data)
  });
...
export async function updateCashRegisterEntry(id: string, data: { type: string; amount: number; note?: string; reference?: string }): Promise<unknown> {
