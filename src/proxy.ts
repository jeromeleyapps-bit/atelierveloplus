...
export async function proxy(request: NextRequest) {
  ...
  if (!pathname.startsWith('/api/')) {
    ...
    // Vérification licence AVANT accès pages admin (Phase 2.1 - Grace Period Option C)
    // Si licence expirée (grace period terminée), rediriger vers page blocage
    ...
    if (isAdminPage && !isAuthPage) {
      try {
        ...
        // Si status = 'expired', bloquer accès et rediriger
        if (licenseInfo.status === 'expired') {
          const url = request.nextUrl.clone();
          url.pathname = '/admin/license/blocked';
          return NextResponse.redirect(url);
        }
      } catch (error) {
        // En cas d'erreur lecture licence, laisser passer (éviter blocage complet)
        logger.error('[Proxy] License check error:', error);
      }
    }
...
export const config = {
  matcher: [
    // Exécuter sur toutes les routes pour permettre la redirection rdv subdomain,
    // mais la logique interne ne traite en auth que /api/*
    '/:path*',
  ],
};
