...
# Version simplifiée sans regex complexes
# ============================================================================
...
if ($configContent -like "*mac:*target:*dmg*") {
...
try {
    $postbuildOutput = npm run postbuild 2>&1
    if ($LASTEXITCODE -ne 0) {
        Write-Host $postbuildOutput
